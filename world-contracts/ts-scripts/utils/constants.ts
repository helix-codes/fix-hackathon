...
type TestResources = {
    ...
    plannerResources?: Array<{
        label: string;
        typeId: number;
        itemId: number;
        volume: number;
        quantity: number;
        inventory?: "storage_owner" | "character_owned";
        approximateRichness?: string;
        freshnessHours?: number;
        exactCoordinates?: string;
        fullScanPayload?: string;
...
export type PlannerResourceSeed = {
    label: string;
    typeId: bigint;
    itemId: bigint;
    volume: bigint;
...
export const PLANNER_RESOURCES: PlannerResourceSeed[] = (res.plannerResources ?? []).map(
    (resource) => ({
        label: resource.label,
        typeId: BigInt(resource.typeId),
        itemId: BigInt(resource.itemId),
        volume: BigInt(resource.volume),
        quantity: resource.quantity,
        inventory: resource.inventory ?? "storage_owner",
        approximateRichness: resource.approximateRichness ?? "Medium",
        freshnessHours: resource.freshnessHours ?? 1,
        exactCoordinates: resource.exactCoordinates ?? "0.0,0.0,0.0",
        fullScanPayload: resource.fullScanPayload ?? "vein-density=unknown",
        recommendedRoute: resource.recommendedRoute ?? "Gate Alpha -> Target Site",
        priceUnits: resource.priceUnits ?? 250,
    })
);
